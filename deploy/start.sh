...
echo "四、创建通道"
docker exec cli peer channel create -o orderer.blockchaindonate.com:7050 -c assetschannel -f /etc/hyperledger/config/assetschannel.tx
...
echo "六、链码安装"
docker exec cli peer chaincode install -n blockchain-donate -v 1.0.0 -l golang -p github.com/togettoyou/blockchain-real-estate/chaincode/blockchain-real-estate
...
docker exec cli peer chaincode instantiate -o orderer.blockchaindonate.com:7050 -C assetschannel -n blockchain-real-estate -l golang -v 1.0.0 -c '{"Args":["init"]}'
...
echo "八、验证查询账户信息"
docker exec cli peer chaincode invoke -C assetschannel -n blockchain-donate -c '{"Args":["queryAccountList"]}'
docker exec cli peer chaincode invoke -C assetschannel -n blockchain-donate -c '{"Args":["queryRealEstateList"]}'
queryRealEstateList

docker exec cli peer chaincode invoke -C assetschannel -n blockchain-donate -c '{"Args":["createRealEstate","5feceb66ffc8", "4b227777d4dd", "0.5", "0.2"]}'